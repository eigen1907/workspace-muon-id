...
def init_model_and_scaler(
    ...
    track_paths = sorted(glob.glob(os.path.join(data_dir, '*.csv')))[:n_files]

    df_list = [pd.read_csv(p) for p in track_paths]
    df = pd.concat(df_list, ignore_index=True)
    df['det_raw_id'] = df['det_raw_id'].apply(lambda x: list(map(int, x.split(' ')[:-1])))
    ...
    detid_encoder.fit(df['det_raw_id'])

    track_scaler = StandardScaler()
    track_scaler.fit(df[['track_pt', 'track_eta', 'track_phi']].values)

    X = track_scaler.transform(df[['track_pt', 'track_eta', 'track_phi']].values)
    Y = detid_encoder.transform(df['det_raw_id'])
    X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size=0.1, random_state=42)

    model = XGBClassifier(n_estimators=n_est, max_depth=n_depth, learning_rate=lr, random_state=42)
...
def main():
    ...
    parser.add_argument('--n_est', type=int, default=100)
