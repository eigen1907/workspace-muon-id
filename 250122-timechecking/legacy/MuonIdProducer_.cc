...
void MuonIdProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // (1) muons first - no cleaning, take as is.
  auto t1start = clock_t::now();
  // -------------------------------------------
  if (muonCollectionHandle_.isValid()) {
  ...
  // -------------------------------------------
  auto t1end = clock_t::now();
  auto block1_time_us = std::chrono::duration_cast<us_t>(t1end - t1start).count();
  hBlock1Time_->Fill(block1_time_us);

  // (2) links second ( assume global muon type )
  auto t2start = clock_t::now();
  // -------------------------------------------
  if (linkCollectionHandle_.isValid()) {
  ...
  // -------------------------------------------
  auto t2end = clock_t::now();
  auto block2_time_us = std::chrono::duration_cast<us_t>(t2end - t2start).count();
  hBlock2Time_->Fill(block2_time_us);

  // (3) tracker and calo muons are next
  auto t3start = clock_t::now();
  // -------------------------------------------
  if (innerTrackCollectionHandle_.isValid()) {
  ...
  // -------------------------------------------
  auto t3end = clock_t::now();
  auto block3_time_us = std::chrono::duration_cast<us_t>(t3end - t3start).count();
  hBlock3Time_->Fill(block3_time_us);

  // (4) and at last the stand alone muons
  auto t4start = clock_t::now();
  // -------------------------------------------
  if (outerTrackCollectionHandle_.isValid()) {
  ...
  // -------------------------------------------
  auto t4end = clock_t::now();
  auto block4_time_us = std::chrono::duration_cast<us_t>(t4end - t4start).count();
  hBlock4Time_->Fill(block4_time_us);

  // (5) (Arbitration, time fill, put 등)
  auto t5start = clock_t::now();
  // -------------------------------------------
  ...
  // -------------------------------------------
  auto t5end = clock_t::now();
  auto block5_time_us = std::chrono::duration_cast<us_t>(t5end - t5start).count();
  hBlock5Time_->Fill(block5_time_us);
}
